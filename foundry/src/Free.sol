...
contract Free {
    ...
    {
        Project storage project = projectById[projectId];
        require(timeToAdd > 0, "The time to add must be greater than 0");

        project.newProposedDeadline = timeToAdd;
        project.newFreelancerQuote = amountToAdd;

        emit NewDeadlineProposed(projectId, timeToAdd, msg.sender, amountToAdd);
    }
        ...
        projectNotFinished(projectId)
    {
        ...
        uint timeRemaining = project.deadline - block.timestamp;
        uint newDeadline = block.timestamp +
            timeRemaining +
            project.newProposedDeadline;
